/*
Naurt API

OpenAPI specification for Naurt's APIs. 

API version: 0.1.4
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package naurt_api

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the FinalDestinationHit type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FinalDestinationHit{}

// FinalDestinationHit struct for FinalDestinationHit
type FinalDestinationHit struct {
	Id string `json:"id"`
	Address string `json:"address"`
	Geojson FinalDestinationHitGeojson `json:"geojson"`
	Distance *float32 `json:"distance,omitempty"`
	// Confidence score in the range 0.0 to 1.0 indicating how well the result  matches the query. Higher is better. This is closely linked to match_level.  See: https://docs.naurt.com/reference/search-confidence  Not to be confused with Accuracy, which is how good the data itself is. This  is about the likelihood of a good match. 
	SearchConfidence *float64 `json:"search_confidence,omitempty"`
	MatchLevel *FinalDestinationMatchLevel `json:"match_level,omitempty"`
	StructuredResponse *StructuredAddress `json:"structured_response,omitempty"`
	SourceId *FinalDestinationSourceIdResponse `json:"source_id,omitempty"`
}

type _FinalDestinationHit FinalDestinationHit

// NewFinalDestinationHit instantiates a new FinalDestinationHit object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFinalDestinationHit(id string, address string, geojson FinalDestinationHitGeojson) *FinalDestinationHit {
	this := FinalDestinationHit{}
	this.Id = id
	this.Address = address
	this.Geojson = geojson
	return &this
}

// NewFinalDestinationHitWithDefaults instantiates a new FinalDestinationHit object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFinalDestinationHitWithDefaults() *FinalDestinationHit {
	this := FinalDestinationHit{}
	return &this
}

// GetId returns the Id field value
func (o *FinalDestinationHit) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *FinalDestinationHit) SetId(v string) {
	o.Id = v
}

// GetAddress returns the Address field value
func (o *FinalDestinationHit) GetAddress() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Address
}

// GetAddressOk returns a tuple with the Address field value
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Address, true
}

// SetAddress sets field value
func (o *FinalDestinationHit) SetAddress(v string) {
	o.Address = v
}

// GetGeojson returns the Geojson field value
func (o *FinalDestinationHit) GetGeojson() FinalDestinationHitGeojson {
	if o == nil {
		var ret FinalDestinationHitGeojson
		return ret
	}

	return o.Geojson
}

// GetGeojsonOk returns a tuple with the Geojson field value
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetGeojsonOk() (*FinalDestinationHitGeojson, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Geojson, true
}

// SetGeojson sets field value
func (o *FinalDestinationHit) SetGeojson(v FinalDestinationHitGeojson) {
	o.Geojson = v
}

// GetDistance returns the Distance field value if set, zero value otherwise.
func (o *FinalDestinationHit) GetDistance() float32 {
	if o == nil || IsNil(o.Distance) {
		var ret float32
		return ret
	}
	return *o.Distance
}

// GetDistanceOk returns a tuple with the Distance field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetDistanceOk() (*float32, bool) {
	if o == nil || IsNil(o.Distance) {
		return nil, false
	}
	return o.Distance, true
}

// HasDistance returns a boolean if a field has been set.
func (o *FinalDestinationHit) HasDistance() bool {
	if o != nil && !IsNil(o.Distance) {
		return true
	}

	return false
}

// SetDistance gets a reference to the given float32 and assigns it to the Distance field.
func (o *FinalDestinationHit) SetDistance(v float32) {
	o.Distance = &v
}

// GetSearchConfidence returns the SearchConfidence field value if set, zero value otherwise.
func (o *FinalDestinationHit) GetSearchConfidence() float64 {
	if o == nil || IsNil(o.SearchConfidence) {
		var ret float64
		return ret
	}
	return *o.SearchConfidence
}

// GetSearchConfidenceOk returns a tuple with the SearchConfidence field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetSearchConfidenceOk() (*float64, bool) {
	if o == nil || IsNil(o.SearchConfidence) {
		return nil, false
	}
	return o.SearchConfidence, true
}

// HasSearchConfidence returns a boolean if a field has been set.
func (o *FinalDestinationHit) HasSearchConfidence() bool {
	if o != nil && !IsNil(o.SearchConfidence) {
		return true
	}

	return false
}

// SetSearchConfidence gets a reference to the given float64 and assigns it to the SearchConfidence field.
func (o *FinalDestinationHit) SetSearchConfidence(v float64) {
	o.SearchConfidence = &v
}

// GetMatchLevel returns the MatchLevel field value if set, zero value otherwise.
func (o *FinalDestinationHit) GetMatchLevel() FinalDestinationMatchLevel {
	if o == nil || IsNil(o.MatchLevel) {
		var ret FinalDestinationMatchLevel
		return ret
	}
	return *o.MatchLevel
}

// GetMatchLevelOk returns a tuple with the MatchLevel field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetMatchLevelOk() (*FinalDestinationMatchLevel, bool) {
	if o == nil || IsNil(o.MatchLevel) {
		return nil, false
	}
	return o.MatchLevel, true
}

// HasMatchLevel returns a boolean if a field has been set.
func (o *FinalDestinationHit) HasMatchLevel() bool {
	if o != nil && !IsNil(o.MatchLevel) {
		return true
	}

	return false
}

// SetMatchLevel gets a reference to the given FinalDestinationMatchLevel and assigns it to the MatchLevel field.
func (o *FinalDestinationHit) SetMatchLevel(v FinalDestinationMatchLevel) {
	o.MatchLevel = &v
}

// GetStructuredResponse returns the StructuredResponse field value if set, zero value otherwise.
func (o *FinalDestinationHit) GetStructuredResponse() StructuredAddress {
	if o == nil || IsNil(o.StructuredResponse) {
		var ret StructuredAddress
		return ret
	}
	return *o.StructuredResponse
}

// GetStructuredResponseOk returns a tuple with the StructuredResponse field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetStructuredResponseOk() (*StructuredAddress, bool) {
	if o == nil || IsNil(o.StructuredResponse) {
		return nil, false
	}
	return o.StructuredResponse, true
}

// HasStructuredResponse returns a boolean if a field has been set.
func (o *FinalDestinationHit) HasStructuredResponse() bool {
	if o != nil && !IsNil(o.StructuredResponse) {
		return true
	}

	return false
}

// SetStructuredResponse gets a reference to the given StructuredAddress and assigns it to the StructuredResponse field.
func (o *FinalDestinationHit) SetStructuredResponse(v StructuredAddress) {
	o.StructuredResponse = &v
}

// GetSourceId returns the SourceId field value if set, zero value otherwise.
func (o *FinalDestinationHit) GetSourceId() FinalDestinationSourceIdResponse {
	if o == nil || IsNil(o.SourceId) {
		var ret FinalDestinationSourceIdResponse
		return ret
	}
	return *o.SourceId
}

// GetSourceIdOk returns a tuple with the SourceId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FinalDestinationHit) GetSourceIdOk() (*FinalDestinationSourceIdResponse, bool) {
	if o == nil || IsNil(o.SourceId) {
		return nil, false
	}
	return o.SourceId, true
}

// HasSourceId returns a boolean if a field has been set.
func (o *FinalDestinationHit) HasSourceId() bool {
	if o != nil && !IsNil(o.SourceId) {
		return true
	}

	return false
}

// SetSourceId gets a reference to the given FinalDestinationSourceIdResponse and assigns it to the SourceId field.
func (o *FinalDestinationHit) SetSourceId(v FinalDestinationSourceIdResponse) {
	o.SourceId = &v
}

func (o FinalDestinationHit) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FinalDestinationHit) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["address"] = o.Address
	toSerialize["geojson"] = o.Geojson
	if !IsNil(o.Distance) {
		toSerialize["distance"] = o.Distance
	}
	if !IsNil(o.SearchConfidence) {
		toSerialize["search_confidence"] = o.SearchConfidence
	}
	if !IsNil(o.MatchLevel) {
		toSerialize["match_level"] = o.MatchLevel
	}
	if !IsNil(o.StructuredResponse) {
		toSerialize["structured_response"] = o.StructuredResponse
	}
	if !IsNil(o.SourceId) {
		toSerialize["source_id"] = o.SourceId
	}
	return toSerialize, nil
}

func (o *FinalDestinationHit) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"address",
		"geojson",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varFinalDestinationHit := _FinalDestinationHit{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varFinalDestinationHit)

	if err != nil {
		return err
	}

	*o = FinalDestinationHit(varFinalDestinationHit)

	return err
}

type NullableFinalDestinationHit struct {
	value *FinalDestinationHit
	isSet bool
}

func (v NullableFinalDestinationHit) Get() *FinalDestinationHit {
	return v.value
}

func (v *NullableFinalDestinationHit) Set(val *FinalDestinationHit) {
	v.value = val
	v.isSet = true
}

func (v NullableFinalDestinationHit) IsSet() bool {
	return v.isSet
}

func (v *NullableFinalDestinationHit) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFinalDestinationHit(val *FinalDestinationHit) *NullableFinalDestinationHit {
	return &NullableFinalDestinationHit{value: val, isSet: true}
}

func (v NullableFinalDestinationHit) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFinalDestinationHit) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


